...
namespace Epc.Core.EpcDecoder
{
    public class Sgtin96 : IEpc
    {
        ...
        public Sgtin96Epc Decode(string HexString)
        {
            ...
            epc.Header = EpcUtility.GetEpcHeader((byte)EpcUtility.DecodeInt(HexString, index, 8));
            index += 8;
            epc.Filter = EpcUtility.DecodeInt(HexString, index, 3);
            index += 3;
            epc.Partition = EpcUtility.DecodeInt(HexString, index, 3);
            index += 3;
            ...
            epc.GS1CompanyPrefix = EpcUtility.DecodeIntString(HexString, index, table.GS1CompanyPrefixBits, table.GS1CompanyPrefixDigits);
            index += table.GS1CompanyPrefixBits;
            epc.IndicatorItemRef = EpcUtility.DecodeIntString(HexString, index, table.ItemReferenceBits, table.ItemReferenceDigits);
            index += table.ItemReferenceBits;
            epc.SerialNumber = EpcUtility.DecodeInt(HexString, index, 38);
